...
export function AgentCreateRecommendationPage() {
  ...
  const prefilledForm = useMemo(() => {
    ...
    return {
      ...
      channels: ensureRequiredChannels(recommendedChannels.length > 0 ? recommendedChannels : inferredForm.channels),
    };
  }, [allowedChannels, inferredForm, routePrefill]);
            ...
            <div className="space-y-3">
              {routePrefill ? (
                <div className="rounded-[22px] border border-brand/15 bg-brand-soft/30 px-4 py-4">
                  ...
                  {routePrefill.outreachEmailBody ? (
                    <div className="mt-4 rounded-xl border border-line/80 bg-white p-3">
                      <div className="flex items-center justify-between gap-3">
                        <p className="text-xs uppercase tracking-[0.16em] text-ink-soft">Outreach email draft</p>
                        <button
                          type="button"
                          onClick={() => navigator.clipboard.writeText(routePrefill.outreachEmailBody ?? '')}
                          className="button-secondary px-3 py-1 text-xs"
                        >
                          Copy
                        </button>
                      </div>
                      <pre className="mt-2 whitespace-pre-wrap text-xs leading-6 text-ink">{routePrefill.outreachEmailBody}</pre>
                    </div>
                  ) : null}
                </div>
              ) : null}
