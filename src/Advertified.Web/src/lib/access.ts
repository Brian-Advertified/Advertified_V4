...
export function canOpenPlanning(campaign?: Campaign | null) {
  return Boolean(
    campaign
      && (campaign.lifecycle
        ? campaign.lifecycle.aiStudioAccessState !== 'locked'
        : campaign.aiUnlocked)
...
export function hasCampaignClearedPayment(campaign?: Campaign | null) {
  if (campaign?.lifecycle) {
    return campaign.lifecycle.paymentState === 'paid';
  }
...
export function getCampaignPaymentState(campaign?: Campaign | null): CampaignPaymentState {
  ...
  switch (campaign.lifecycle?.paymentState) {
    case 'under_review':
...
export function getCampaignPrimaryAction(campaign: Campaign) {
  ...
  const paymentRequiredBeforeApproval = getCampaignPaymentState(campaign) === 'payment_required'
