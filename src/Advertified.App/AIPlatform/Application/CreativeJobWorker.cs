...
public sealed class CreativeJobWorker : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                await foreach (var envelope in _queue.DequeueAllAsync(stoppingToken))
                {
                    ...
                    catch (Exception ex)
                    {
                        _logger.LogError(
                            ex,
                            "Creative job worker suppressed an unhandled exception for job {JobId} on campaign {CampaignId}.",
                            envelope.Request.JobId,
                            envelope.Request.Command.CampaignId);
                    }
                }
            }
            catch (OperationCanceledException) when (stoppingToken.IsCancellationRequested)
            ...
            {
                _logger.LogError(ex, "Creative job worker loop failed unexpectedly. Restarting queue pump.");
                await Task.Delay(TimeSpan.FromSeconds(5), stoppingToken);
            }
        }
    }

    private async Task ProcessEnvelopeAsync(QueueCreativeJobEnvelope envelope, CancellationToken stoppingToken)
    {
